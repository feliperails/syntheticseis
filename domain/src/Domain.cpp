...
{    
    ...
    auto fineGrainedSandstone = entityManager.createEntity(QLatin1String("Fine-grained sandstone"), std::make_shared<Lithology>(5));
    auto mediumGrainedSandstone = entityManager.createEntity(QLatin1String("Medium-grained sandstone"), std::make_shared<Lithology>(7));
    auto coarseGrainedSandstone = entityManager.createEntity(QLatin1String("Coarse-grained sandstone"), std::make_shared<Lithology>(9));

    // No paper também está 7, qual o valor correto?
    auto veryCoarseGrained = entityManager.createEntity(QLatin1String("Very coarse-grained"), std::make_shared<Lithology>(7.5));
    auto conglomerate = entityManager.createEntity(QLatin1String("Conglomerate"), std::make_shared<Lithology>(13));
    ...
    seismicWaveVelocityDictionary().setVelocity(mediumGrainedSandstone, 3200.0); // m/s
    seismicWaveVelocityDictionary().setVelocity(coarseGrainedSandstone, 3500.0); // m/s
    seismicWaveVelocityDictionary().setVelocity(veryCoarseGrained, 3700.0); // m/s
    seismicWaveVelocityDictionary().setVelocity(conglomerate, 4000.0); // m/s
